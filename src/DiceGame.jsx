...
// 红六勃：6个4
if (counts[4] === 6) {
  return { name: '红六勃', level: 10, description: '六个四，状元及第，至尊荣耀！', color: '#dc2626' };
}

// 遍地锦：6个1
if (counts[1] === 6) {
  return { name: '遍地锦', level: 9, description: '六个一，状元及第，极致稀有！', color: '#dc2626' };
}

// 黑六勃：6个2
if (counts[2] === 6) {
  return { name: '黑六勃', level: 8, description: '六个二，状元及第，独一无二！', color: '#1f2937' };
}

// 插金花：4个4 + 2个1
if (counts[4] === 4 && counts[1] === 2) {
  return { name: '插金花', level: 7, description: '四个四加两个一，状元及第，锦上添花！', color: '#f59e0b' };
}

// 五红：5个4
if (counts[4] === 5) {
  return { name: '五红', level: 6, description: '五个四，状元及第，鸿运当头！', color: '#dc2626' };
}

// 五子登科：5个2
if (counts[2] === 5) {
  return { name: '五子', level: 6, description: '五个二，状元及第，五子登科！', color: '#dc2626' };
}

// 四红：4个4
if (counts[4] === 4) {
  return { name: '四红', level: 5, description: '四个四，状元及第，运势非凡！', color: '#dc2626' };
}

// 榜眼：123456顺子
const hasAllNumbers = [1, 2, 3, 4, 5, 6].every(num => counts[num] === 1);
if (hasAllNumbers) {
  return { name: '榜眼', level: 4, description: '顺子齐聚，才华横溢，榜眼之选！', color: '#7c3aed' };
}

// 探花：3个4
if (counts[4] === 3) {
  return { name: '探花', level: 3, description: '三个四，风华出众，探花之姿！', color: '#dc2626' };
}

// 进士：4个2
if (counts[2] === 4) {
  return { name: '进士', level: 2, description: '四个二，才学兼备，进士及第！', color: '#1f2937' };
}

// 举人：2个4
if (counts[4] === 2) {
  return { name: '举人', level: 1, description: '两个四，实力不凡，稳入举人！', color: '#dc2626' };
}

// 秀才：1个4
if (counts[4] === 1) {
  return { name: '秀才', level: 0, description: '一个四，初露锋芒，秀才入门！', color: '#dc2626' };
}

// 无奖
return { name: '无奖', level: -1, description: '未中佳手，再接再厉！', color: '#6b7280' };
};
          ...
          <div className="result-section">
            ...
            {gameResult && (
              <div className="bo-result" style={{ borderColor: gameResult.color }}>
                ...
                {gameResult.name === '红六勃' && <div className="celebration">🎊 恭喜高中状元！ 🎊</div>}
                {gameResult.name === '插金花' && <div className="celebration">✨ 极品奖励！ ✨</div>}
              </div>
            )}
          </div>
        )}
